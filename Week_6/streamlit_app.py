...
sayfa = st.sidebar.selectbox(
    "Analiz türünü seçin:",
    ["🏠 Ana Sayfa", "📊 Veri Görüntüleme", "🔍 Popüler Ürünler", "🔗 Birliktelik Analizi", "📋 Kural Analizi", "🎯 Ürün Önerileri", "🆕 Yeni Sayfam"]
)
...
if veri is not None and sepetler is not None:
    ...
    if sayfa == "🏠 Ana Sayfa":
        ...
        **Ana Kavramlar:**
        ...
        st.write("İlk 5 sepet:")
        st.dataframe(veri.head(5))
    ...
    elif sayfa == "🆕 Yeni Sayfam":  # ← BURAYA EKLEME
        st.header("📊 Ürün Dağılımı Analizi")
        st.write("En popüler ürünlerin pasta grafiği")
    
        # En popüler 8 ürünü al
        sorted_urunler = sorted(urun_sayilari.items(), key=lambda x: x[1], reverse=True)
        top_8 = sorted_urunler[:8]
    
        # Veriyi hazırla
        urunler = [item[0] for item in top_8]
        sayilar = [item[1] for item in top_8]
    
        # Pie Chart oluştur
        fig = px.pie(
            values=sayilar,
            names=urunler,
            title='En Popüler 8 Ürünün Sepet Dağılımı',
            hole=0.3,  # Donut chart için (istemezsen 0 yap)
        ...
        for i, (urun, sayi) in enumerate(top_8, 1):
            yuzde = (sayi / len(sepetler)) * 100
            st.write(f"{i}. **{urun}**: {sayi} sepet (%{yuzde:.1f})")

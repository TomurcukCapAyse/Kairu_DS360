...
class TimeSeriesCV:
    ...
    def __init__(self, artifacts_path=None, validation_horizon=28, n_splits=3):
        if artifacts_path is None:
            script_dir = os.path.dirname(os.path.abspath(__file__))
            artifacts_path = os.path.join(os.path.dirname(script_dir), 'P2_feature_engineering', 'artifacts')
    ...
    def load_data(self):
        ...
        try:
            train_path = os.path.join(self.artifacts_path, 'datasets', 'fe_train.parquet')
            valid_path = os.path.join(self.artifacts_path, 'datasets', 'fe_valid.parquet')

            print(f"   📍 Train path: {train_path}")

            if not os.path.exists(train_path):
                raise FileNotFoundError(f"❌ Dosya bulunamadı: {train_path}")
            if not os.path.exists(valid_path):
                raise FileNotFoundError(f"❌ Dosya bulunamadı: {valid_path}")
    ...
    def generate_reports(self):
        ...
        json_path = './artifacts/reports/tscv_report.json'
        ...
        md_path = './artifacts/reports/tscv_report.md'
...
"""
    ...
    def run_full_pipeline(self):
        ...
        try:
            ...
            print(f"📁 Raporlar: ./artifacts/reports/")

...
def create_features():
    ...
    print("\n📁 Çıktı klasörleri hazırlanıyor...")
    output_dirs = [
        './P2_feature_engineering/artifacts/datasets',
        './P2_feature_engineering/artifacts/figures'
    ]
    ...
    try:
        # Train verisi
        train_df = pd.read_csv('./P1_data_preparation/artifacts/datasets/train.csv', parse_dates=['date'], index_col='date')
        print(f"   ✓ Train verisi: {train_df.shape}")
        
        # Validation verisi
        valid_df = pd.read_csv('./P1_data_preparation/artifacts/datasets/valid.csv', parse_dates=['date'], index_col='date')
    ...
    for item_id in all_df['item_id'].unique():
        ...
        # Store_id varsa koru, yoksa varsayılan değer ata
        if 'store_id' in all_df.columns:
            item_df['store_id'] = item_df['store_id'].iloc[0]
        else:
            item_df['store_id'] = 'CA_1'  # Varsayılan store
    ...
    # Rolling özellikler: NaN olmamalı
    roll_features = ['roll_mean_7', 'roll_mean_28']  # Groupby öneri
    for roll_col in roll_features:
        if roll_col in feature_df.columns:
            before_count = feature_df[roll_col].isnull().sum()

            if before_count > 0:
                # Her ürün kendi geçmişiyle doldurur (grup bazlı) 
                feature_df[roll_col] = feature_df.groupby('item_id', group_keys=False)[roll_col].apply(lambda x: x.fillna().fillna(0))
                
                after_count = feature_df[roll_col].isnull().sum()
                print(f"     - {roll_col}: {before_count} → {after_count} NaN")
            
            else:
                print(f"     - {roll_col}: NaN yok , işlem atlandı (doldurma gerekmedi)")
    
    # Final kontrol
    final_nan = feature_df.isnull().sum().sum()  # Hem satır hem sütun bazında toplam NaN
    ...
    fe_train_path = './P2_feature_engineering/artifacts/datasets/fe_train.parquet'
    fe_valid_path = './P2_feature_engineering/artifacts/datasets/fe_valid.parquet'
    ...
    X_train.to_parquet('./P2_feature_engineering/artifacts/datasets/X_train.parquet')
    y_train.to_frame('sales').to_parquet('./P2_feature_engineering/artifacts/datasets/y_train.parquet')
    X_valid.to_parquet('./P2_feature_engineering/artifacts/datasets/X_valid.parquet')
    y_valid.to_frame('sales').to_parquet('./P2_feature_engineering/artifacts/datasets/y_valid.parquet')
    ...
    # Kaydet
    hist_path = './P2_feature_engineering/artifacts/figures/feature_distributions.png'
    ...
    # Kaydet
    corr_path = './P2_feature_engineering/artifacts/figures/feature_correlations.png'
    ...
    print(f"📁 Çıktılar: ./P2_feature_engineering/artifacts/datasets/ ve ./P2_feature_engineering/artifacts/figures/")

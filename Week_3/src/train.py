...
# artifacts/ yoksa oluştur; varsa devam et hata vermeden
def ensure_dir():
...
# Azınlık sınıfı dengelemek için bu işlemi yaparız.
...
# Her modelin başarısını değerlendirir ve yazar.
...
# Önemli: Eğitimde kullanılan kolon/isim sırası kaydedilir. Üretimde "X has N features..." hataları önler.
    ...
    joblib.dump(xgb, f"{ARTIFACTS_DIR}/model_xgb{save_tag}.pkl")   # Veriye dokunmadan, modeli ağırlıklarla adil davranmaya zorluyoruz.
...
# pre işlemi sadece train'de fit edilir (testi hiç görmez).
...
# Böylece leakage yok.
    ...
    # Test -> Sadece transform işlemi yap , fit yok sonuçlar yazdırılır.
    ...
    # XGB de resampled train ile eğitilir (bu sefer scale_pos_weight yok; zaten veri dengelenmiş durumda).

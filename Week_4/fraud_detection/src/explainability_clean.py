...
class ModelExplainer:
    ...
    def compute_shap_values(self, X_test, max_samples=50):
        ...
        try:
            # SHAP API differs by version; try common patterns
            try:
                shap_values = self.shap_explainer.shap_values(X_sample)
            except Exception:
                # newer shap uses callables
    ...
    def plot_shap_summary(self, X_sample, shap_values=None, max_display=10):
        """SHAP summary plot (beeswarm)"""
        if not SHAP_AVAILABLE:
            logger.warning("SHAP mevcut değil, plot atlandı")
            return None

        try:
            if shap_values is None:
                shap_values, X_sample = self.compute_shap_values(X_sample)

            plt.figure(figsize=(10, 6))
            # shap.summary_plot handles different shapes/versions
            shap.summary_plot(shap_values, X_sample, feature_names=self.feature_names, max_display=max_display, show=True)
            plt.tight_layout()
            return True
        except Exception as e:
    ...
    def compute_permutation_importance(self, X_test, y_test):
    ...
    # Backwards-compatible alias (some demos may call the old name)
    def permutation_importance(self, X_test, y_test):
        return self.compute_permutation_importance(X_test, y_test)
